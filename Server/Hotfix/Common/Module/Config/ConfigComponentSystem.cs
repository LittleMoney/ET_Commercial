    ...
    public static class ConfigComponentHelper
	{
		...
		public static void Load(this ConfigComponent self)
		{
			...
			foreach (Type type in types)
			{
				...
				if (iCategory == null)
				{
					throw new Exception($"class: {type.FullName} not inherit from ACategory");
				}
				iCategory.BeginInit(ConfigHelper.GetText(type.Name.Substring(0, type.Name.IndexOf("Category"))));
				iCategory.EndInit();
