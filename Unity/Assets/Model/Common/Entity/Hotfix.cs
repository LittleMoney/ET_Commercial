...
namespace ETModel
{
	public sealed class Hotfix: Object
	{
		...
		{
			byte[] assBytes = (ResourcesComponent.Instance.LoadAsset("Common/Hotfix.dll", typeof(UnityEngine.TextAsset)) as TextAsset).bytes;
			byte[] pdbBytes = (ResourcesComponent.Instance.LoadAsset("Common/Hotfix.pdb", typeof(UnityEngine.TextAsset)) as TextAsset).bytes;
			...
			Game.Scene.GetComponent<ResourcesComponent>().UnloadBundle($"Common/Hotfix.dll");
			Game.Scene.GetComponent<ResourcesComponent>().UnloadBundle($"Common/Hotfix.pdb");
		}
	}
}