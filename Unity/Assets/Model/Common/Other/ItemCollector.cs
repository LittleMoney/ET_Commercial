...
[Serializable]
public class ItemCollectorData
{
	/// <summary>
	/// 组件类型
	/// </summary>
...
public class ItemCollectorDataComparer : IComparer<ItemCollectorData>
{
	public int Compare(ItemCollectorData x, ItemCollectorData y)
	{
...
public class ItemCollector : MonoBehaviour, ISerializationCallbackReceiver, IEnumerable<ItemCollectorData>
{
	//用于序列化的List
	public List<ItemCollectorData> data = new List<ItemCollectorData>();
	//Object并非C#基础中的Object，而是 UnityEngine.Object
	private readonly Dictionary<string, ItemCollectorData> dict = new Dictionary<string, ItemCollectorData>();
	...
	{
		SerializedObject serializedObject = new SerializedObject(this);
		data.Sort(new ItemCollectorDataComparer());
		EditorUtility.SetDirty(this);
		serializedObject.ApplyModifiedProperties();
		serializedObject.UpdateIfRequiredOrScript();
	}
	...
	public Object this[string key]
	{
		get
		{
			if (dict.TryGetValue(key,  out ItemCollectorData dataItem))
			{
				return dataItem.obj;
			}
			return null;
	...
	public ItemType GetType(string key)
	{
		if (dict.TryGetValue(key, out ItemCollectorData dataItem))
		{
	...
	{
		dict.Clear();
		foreach (ItemCollectorData itemCollectorData in data)
		{
			if (!dict.ContainsKey(itemCollectorData.key))
			{
				dict.Add(itemCollectorData.key, itemCollectorData);
			}
		}
	}

	public IEnumerator<ItemCollectorData> GetEnumerator()
	{
