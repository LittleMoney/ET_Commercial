    ...
    public class UIDefaultFactory : IUIFactory
    {
		...
		public UI Create(string uiType, GameObject uiRoot, object data)
		{
			try
			{
				...
				Type _type = Type.GetType($"ETModel.{uiType.GetPureFileName()}");
				if(_type==null) _type=Type.GetType($"ETModel.{uiType.GetPureFileName()}Component");
			...
			{
				throw new Exception($"打开ui错误:{uiType} {e.Message}");
			}
		}
