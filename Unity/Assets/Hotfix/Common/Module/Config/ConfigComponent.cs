...
namespace ETHotfix
{
	...
	public class ConfigComponent: Component
	{
		...
		public  ACategory Load(string path)
		{
			...
			TextAsset _textAsset =  resourcesComponent.LoadAsset(path, typeof(TextAsset)) as TextAsset;

			{
				Type _type = Type.GetType($"ETModel.{path.GetPureFileName()}Category");

				if (_type == null || !_type.IsAssignableFrom(typeof(ACategory)))
				{
			...
			resourcesComponent.UnloadBundle(path);
		...
		public async ETTask<ACategory> LoadAsync(string path)
		{
			...
			TextAsset _textAsset= await resourcesComponent.LoadAssetAsync(path,typeof(TextAsset)) as TextAsset;
